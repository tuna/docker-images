...
func parseCLIParams() config {
	...
	var bufferUnitSize UnitValue = UnitValue(1 * 1024 * 1024) // Default to 1MB
...
func handleConnection(conn *net.TCPConn, cfg config) {
	...
	go func() { //Downloading data from upstream to client, with buffering
		...
		const maxInMemBuffers = 4
		thisConnBufferChannel := make(chan struct{}, maxInMemBuffers)
		go func() { // Sender
			defer wg.Done()
			closed := false
		...
		go func() { // Receiver
			...
			for {
				...
				var inMem bool
				select {
				case thisConnBufferChannel <- struct{}{}:
					for {
